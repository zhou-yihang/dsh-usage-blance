...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PREFS_PATH = '/api/dsh-usage/prefs'
    ...
    var positionStore = createPrefStore(loadPosition, function (value) {
      savePosition(value)
      pushPrefs()
    })
    var glassStore = createPrefStore(loadGlass, function (value) {
      saveGlass(value)
      pushPrefs()
    })
    var alertStore = createPrefStore(loadBalanceAlert, function (value) {
      saveBalanceAlert(value)
      pushPrefs()
    })

    // Host-side persistence: localStorage alone proved unreliable in some
    // setups (cleared on restart), so the full prefs object is also written
    // to the host state file via a debounced POST, and loaded back once per
    // bundle load (host values win — they survive browser restarts).
    var pushPrefsTimer = null
    function pushPrefs() {
      if (pushPrefsTimer) clearTimeout(pushPrefsTimer)
      pushPrefsTimer = setTimeout(function () {
        pushPrefsTimer = null
        try {
          fetch(PREFS_PATH, {
            method: 'POST',
            ...
            body: JSON.stringify({
              prefs: {
                ...
                glass: glassStore.get(),
                balanceAlert: alertStore.get()
              }
            })
    ...
    function mergeHostPrefs(prefs) {
      if (!prefs || typeof prefs !== 'object') return
      if (prefs.position === 'above' || prefs.position === 'below') positionStore.set(prefs.position)
      if (typeof prefs.balanceAlert === 'number' && Number.isFinite(prefs.balanceAlert) && prefs.balanceAlert >= 0) {
        alertStore.set(prefs.balanceAlert)
      } else if (prefs.balanceAlert === null) {
        alertStore.set(null)
      }
      if (prefs.glass && typeof prefs.glass === 'object') {
        glassStore.set({
          enabled: prefs.glass.enabled !== false,
          opacity: clampNumber(prefs.glass.opacity, 20, 90, 52),
          blur: clampNumber(prefs.glass.blur, 0, 32, 16),
          saturate: clampNumber(prefs.glass.saturate, 0.5, 2, 1.4)
        })
      }
    }
    ...
    var stripStyle = {
      ...
      // The glass backdrop-filter turns the strip into a stacking context
      // (z auto ≈ 0); without an explicit z-index, the panel inside it —
      // whose z-index is local to that context — paints BELOW elements
      // with positive z-index inside the composer card (e.g. in the hero
      // fallback, where the strip flows after the input via flex order).
      // Elevating the strip lifts its whole context above the card.
      zIndex: 40,
    ...
    // Sticky header: the panel itself scrolls (overflow-y auto), so pin the
    // title + close button to the panel's top-right while the columns scroll
    // underneath. Negative margins neutralize the panel padding; the header
    // carries its own padding + background + bottom hairline.
    ...
    function apply(ctx) {
      try {
        // Load host-persisted preferences once per bundle load; they win
        // over localStorage so the alert/position/glass settings survive
        // browser restarts and storage clears.
        try {
          fetch(PREFS_PATH, { cache: 'no-store' })
            .then(function (res) { return res.json() })
            .then(function (body) { if (body && body.ok) mergeHostPrefs(body.prefs) })
            .catch(function () { /* ignore — localStorage fallback stands */ })
        } catch (e) { /* ignore */ }
