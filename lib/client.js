...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      try {
        // Keyframes for the refresh icon; tagged so the module system can
        // reclaim it on HMR invalidate (same contract as bundled CSS).
        if (typeof document !== 'undefined' && !document.querySelector('style[data-plugin="dsh-usage-blance"]')) {
          ...
          style.textContent = '@keyframes dsh-usage-blance-spin{to{transform:rotate(360deg)}}'
