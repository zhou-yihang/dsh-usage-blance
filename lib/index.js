 ...
 * The file holds both the userToken AND the UI prefs (position / glass /
 * balance alert) — either may exist without the other.
 */
function readState() {
  try {
...
/** Merge a patch into the persisted state (atomic best-effort write). */
function updateState(patch) {
  const current = readState() ?? {}
  return writeState({ ...current, ...patch })
}

/** A saved userToken is configured; an empty/absent one is not. */
function hasToken(state) {
  return typeof state?.userToken === 'string' && state.userToken !== ''
}
...
async function handleOverview(_req, res) {
  ...
  const out = {
    ok: true,
    at: now,
    tokenConfigured: hasToken(readState()),
    balance: null,
    balanceError: null,
    usage: null,
    usageError: null
  ...
  try {
    ...
    if (!hasToken(state)) throw new NoTokenError()
...
async function handleToken(req, res) {
  ...
  if (method === 'GET') {
    ...
    sendJson(res, 200, {
      ok: true,
      configured: hasToken(state),
      masked: hasToken(state) ? maskToken(state.userToken) : ''
    })
    return
  }

  if (method === 'DELETE') {
    updateState({ userToken: '', clearedAt: Date.now() })
    cache.usage = null
  ...
  if (method === 'POST') {
    ...
    if (token === '') {
      // Empty token = clear (prefs preserved).
      updateState({ userToken: '', clearedAt: Date.now() })
      cache.usage = null
      sendJson(res, 200, { ok: true, configured: false })
      return
    }
    updateState({ userToken: token, updatedAt: Date.now() })
    cache.usage = null
...
// ---- UI preferences (host-persisted; survive browser storage loss) -------

function clampPref(value, min, max, fallback) {
  const n = Number(value)
...
/**
 * Normalize an untrusted prefs object to the known keys with sane bounds.
 * Unknown keys are dropped.
 */
export function sanitizePrefs(input) {
  const prefs = {}
  if (input === null || typeof input !== 'object') return prefs
  if (input.position === 'above' || input.position === 'below') prefs.position = input.position
  if (typeof input.balanceAlert === 'number' && Number.isFinite(input.balanceAlert) && input.balanceAlert >= 0) {
    prefs.balanceAlert = input.balanceAlert
  } else if (input.balanceAlert === null) {
    prefs.balanceAlert = null
  }
  if (input.glass !== null && typeof input.glass === 'object') {
    const glass = {
      enabled: input.glass.enabled !== false,
      opacity: Math.round(clampPref(input.glass.opacity, 20, 90, 52)),
      blur: Math.round(clampPref(input.glass.blur, 0, 32, 16)),
      saturate: clampPref(input.glass.saturate, 0.5, 2, 1.4)
    }
    prefs.glass = glass
  }
  return prefs
...
/**
 * GET/POST /api/dsh-usage/prefs — the client writes UI preferences through
 * here so they live in $DSH_HOME/storages/dsh-usage-blance.json and survive
 * browser restarts / storage clears (localStorage alone proved unreliable in
 * some setups). POST merges only sanitized known keys.
 */
async function handlePrefs(req, res) {
  ...
  if (method === 'GET') {
    const state = readState()
    sendJson(res, 200, { ok: true, prefs: state?.prefs ?? {} })
    return
  }
  ...
  if (method === 'POST') {
    ...
    const incoming = sanitizePrefs(body.prefs)
    const state = readState() ?? {}
    const prefs = { ...(state.prefs ?? {}), ...incoming }
    updateState({ prefs })
...
export function apply(ctx) {
  ctx.effect(() => {
    ...
    const disposePrefs = ctx.webServer.register({
      kind: 'exact',
      path: '/api/dsh-usage/prefs',
      handler: guarded(handlePrefs)
    })
    return () => {
      disposeOverview()
      disposeToken()
